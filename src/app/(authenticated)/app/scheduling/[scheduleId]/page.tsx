...
export default async function ScheduleDetailsPage({ params, searchParams }: PageProps<"/app/scheduling/[scheduleId]"> & { searchParams: Promise<{ week?: string; view?: string; day?: string; copy?: string; revision?: string }> }) {
  ...
  const revision = selectScheduleRevision(revisions, query.revision);
  const officialPublishedRevision = selectOfficialPublishedRevision(revisions);
  ...
  const actorLabel = (userId: string | null) => {
    if (!userId) return "—";
    if (userId === authorization.userId) return "Você";
    return actorNames.get(userId) ?? "Membro da equipe";
  };
  const view = query.view === "day" || query.view === "worker" ? query.view : "weekly";
  const revisionQuery = revision ? `&revision=${revision.id}` : "";
  const weekStart = resolveScheduleWeekStart(schedule.period_start, schedule.period_end, query.week);
  const weekLink = (week: string) => `/app/scheduling/${schedule.id}?view=weekly&week=${week}${revisionQuery}`;
  ...
  const dayLink = (day: string) => `/app/scheduling/${schedule.id}?view=day&day=${day}${revisionQuery}`;
  ...
  const canCreateAbsence = canRegisterAbsenceOnRevision(permissions.has("absence:create"), revision, revisions);
  const revisionGuidance = revision?.status === "draft"
    ? "Confira as jornadas antes de enviar esta versão para aprovação."
    : revision?.status === "pending_approval"
      ? "Esta versão aguarda aprovação ou devolução para ajustes."
      : revision?.status === "approved"
        ? "A versão está aprovada. A publicação tornará suas jornadas a referência oficial da operação."
        : "Esta versão foi publicada. Para ajustar jornadas, crie uma nova revisão.";
  const hasRevisionAudit = Boolean(revision?.submitted_at || revision?.approved_at || revision?.published_at);
  return <PageShell><ContentContainer size="detail-wide"><PageHeader breadcrumb={<Breadcrumb items={[{ label: "Operação" }, { label: "Escalas", href: "/app/scheduling" }, { label: `${formatCivilDate(schedule.period_start)} — ${formatCivilDate(schedule.period_end)}` }]} />} description="Organize as jornadas e acompanhe a aprovação da escala." metadata={revision ? <div className="flex items-center gap-3"><ScheduleStatusBadge status={revision.status} /><span className="text-sm text-muted-foreground">Versão {revision.version}</span>{revision.id === officialPublishedRevision?.id ? <span className="rounded-full border border-border-default px-2 py-0.5 text-xs font-medium">Publicada oficial</span> : null}</div> : undefined} title="Escala" />
    ...
    <div className="mt-8 divide-y divide-border-default border-y border-border-default">
      <section className="py-6"><h2 className="font-semibold">Resumo</h2><dl className="mt-5 grid gap-x-8 gap-y-5 sm:grid-cols-2 lg:grid-cols-3"><Item label="Operação" value={operation.name} /><Item label="Período" value={`${formatCivilDate(schedule.period_start)} — ${formatCivilDate(schedule.period_end)}`} /><Item label="Jornadas" value={revisionWithEntries?.entries.length ?? 0} /></dl></section>
      {revision && revisionWithEntries ? <><nav aria-label="Visualização da programação" className="flex gap-2 py-4"><Button asChild size="sm" variant={view === "weekly" ? "default" : "outline"}><Link href={weekLink(weekStart)}>Semanal</Link></Button><Button asChild size="sm" variant={view === "day" ? "default" : "outline"}><Link href={dayLink(selectedDay)}>Dia</Link></Button><Button asChild size="sm" variant={view === "worker" ? "default" : "outline"}><Link href={`/app/scheduling/${schedule.id}?view=worker${revisionQuery}`}>Colaborador</Link></Button></nav>{view === "weekly" ? <ScheduleWeekEditor assignments={assignments} canCreateAbsence={canCreateAbsence} entries={revisionWithEntries.entries} initialHref={weekLink(resolveScheduleWeekStart(schedule.period_start, schedule.period_end))} nextHref={nextWeek} periodEnd={schedule.period_end} periodStart={schedule.period_start} previousHref={previousWeek} revisionId={revision.id} scheduleId={schedule.id} status={revision.status} weekStart={weekStart} /> : null}{view === "day" ? <ScheduleWeekEditor assignments={assignments} canCreateAbsence={canCreateAbsence} days={scheduleWeekDays(selectedDay).filter((day) => day.key === selectedDay)} entries={revisionWithEntries.entries} initialHref={dayLink(schedule.period_start)} initialLabel="Início do período" nextHref={nextDay} nextLabel="Próximo dia" periodEnd={schedule.period_end} periodStart={schedule.period_start} previousHref={previousDay} previousLabel="Dia anterior" revisionId={revision.id} scheduleId={schedule.id} status={revision.status} title="Programação do dia" weekStart={selectedDay} /> : null}{view === "worker" ? <ScheduleWorkerView canCreateAbsence={canCreateAbsence} entries={revisionWithEntries.entries} scheduleId={schedule.id} /> : null}</> : null}
      {revision ? <section className="py-6"><div><h2 className="font-semibold">Aprovação e publicação</h2><p className="mt-1 text-sm leading-6 text-muted-foreground">{revisionGuidance}</p></div><div className="mt-4"><ScheduleRevisionActions permissions={permissions} revisionId={revision.id} scheduleId={schedule.id} status={revision.status} /></div>{hasRevisionAudit ? <details className="group mt-5"><summary className="cursor-pointer text-sm font-medium text-action-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus-ring">Ver datas e responsáveis</summary><dl className="mt-4 grid gap-x-8 gap-y-5 sm:grid-cols-2 lg:grid-cols-3">{revision.submitted_at ? <><Item label="Enviada em" value={formatDate(revision.submitted_at)} /><Item label="Enviada por" value={actorLabel(revision.submitted_by)} /></> : null}{revision.approved_at ? <><Item label="Aprovada em" value={formatDate(revision.approved_at)} /><Item label="Aprovada por" value={actorLabel(revision.approved_by)} /></> : null}{revision.published_at ? <><Item label="Publicada em" value={formatDate(revision.published_at)} /><Item label="Publicada por" value={actorLabel(revision.published_by)} /></> : null}</dl></details> : null}</section> : null}
      {revisions.length > 1 ? <details className="group py-6"><summary className="cursor-pointer font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus-ring">Histórico de revisões <span className="ml-1 text-sm font-normal text-muted-foreground">({revisions.length} versões)</span></summary><div className="mt-5 space-y-3">{revisions.map((item) => <div className="flex flex-wrap items-center justify-between gap-3 rounded-surface border border-border-default px-4 py-3" key={item.id}><div><p className="font-medium">Versão {item.version}{item.id === officialPublishedRevision?.id ? " · Publicada oficial" : ""}</p><p className="mt-1 text-sm text-muted-foreground">Criada em {formatDate(item.created_at)}</p></div><div className="flex items-center gap-2"><ScheduleStatusBadge status={item.status} /><Button asChild size="sm" variant="ghost"><Link href={`/app/scheduling/${schedule.id}?revision=${item.id}`}>Visualizar</Link></Button></div></div>)}</div></details> : null}
    </div>
