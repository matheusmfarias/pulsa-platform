...
export default async function ScheduleDetailsPage({ params, searchParams }: PageProps<"/app/scheduling/[scheduleId]"> & { searchParams: Promise<{ week?: string; view?: string; day?: string; copy?: string; revision?: string }> }) {
  ...
  return <PageShell><ContentContainer size="detail-wide"><PageHeader breadcrumb={<Breadcrumb items={[{ label: "Operação" }, { label: "Escalas", href: "/app/scheduling" }, { label: `${formatCivilDate(schedule.period_start)} — ${formatCivilDate(schedule.period_end)}` }]} />} description="Revisões, programação semanal e lifecycle da escala." metadata={revision ? <div className="flex items-center gap-3"><ScheduleStatusBadge status={revision.status} /><span className="text-sm text-muted-foreground">Versão {revision.version}</span>{revision.id === officialPublishedRevision?.id ? <span className="rounded-full border border-border-default px-2 py-0.5 text-xs font-medium">Publicada oficial</span> : null}</div> : undefined} title="Escala" />
    ...
    <div className="mt-8 divide-y divide-border-default border-y border-border-default">
      ...
      {revision ? <section className="py-6"><div><h2 className="font-semibold">Aprovação e publicação</h2><p className="mt-1 text-sm leading-6 text-muted-foreground">{revisionGuidance}</p></div><div className="mt-4"><ScheduleRevisionActions permissions={permissions} revisionId={revision.id} scheduleId={schedule.id} status={revision.status} /></div><dl className="mt-5 grid gap-x-8 gap-y-5 sm:grid-cols-2 lg:grid-cols-3"><Item label="Enviada em" value={formatDate(revision.submitted_at)} /><Item label="Aprovada em" value={formatDate(revision.approved_at)} /><Item label="Publicada em" value={formatDate(revision.published_at)} /><Item label="Enviada por" value={actorLabel(revision.submitted_by)} /><Item label="Aprovada por" value={actorLabel(revision.approved_by)} /><Item label="Publicada por" value={actorLabel(revision.published_by)} /></dl></section> : null}
      <section className="py-6"><h2 className="font-semibold">Histórico de revisões</h2><div className="mt-5 space-y-3">{revisions.map((item) => <div className="flex flex-wrap items-center justify-between gap-3 rounded-surface border border-border-default px-4 py-3" key={item.id}><div><p className="font-medium">Versão {item.version}{item.id === officialPublishedRevision?.id ? " · Publicada oficial" : ""}</p><p className="mt-1 text-sm text-muted-foreground">Criada em {formatDate(item.created_at)}</p></div><div className="flex items-center gap-2"><ScheduleStatusBadge status={item.status} /><Button asChild size="sm" variant="ghost"><Link href={`/app/scheduling/${schedule.id}?revision=${item.id}`}>Visualizar</Link></Button></div></div>)}</div></section>
    </div>
