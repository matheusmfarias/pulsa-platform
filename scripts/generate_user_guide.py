...
def page(canvas, doc):
    ...
    canvas.drawString(LEFT, 21, "Guia para Diretoria de Operações e RH  |  23 set 2026")
...
add("Edição de 23 de setembro de 2026  ·  Aplicação atual", "muted")
...
add("Você pode consultar os módulos pelo menu lateral. No celular, use o botão de menu no alto da tela. Clique no nome de um registro para ver seus detalhes; quando seu perfil permitir, as ações de edição ou mudança de situação aparecem na própria página.")
h2("Entrar e sair")
...
step(2, "Escolha o motivo, inclua apenas a observação necessária e clique em <b>Confirmar</b>. A entrada planejada continua visível, agora com o aviso de ausência.")
...
h2("Visão geral")
add("A <b>Visão geral</b> abre com <b>Requer atenção</b>: por exemplo, ausências sem cobertura, postos abaixo do efetivo base e colaboradores ativos sem alocação. Depois mostra indicadores da estrutura ativa. Clique em uma pendência para investigar a lista correspondente.")
...
h2("Administração")
add("O menu <b>Administração</b> é reservado à Direção. Em <b>Usuários</b>, é possível consultar pessoas já associadas à organização e alterar papel ou situação. A tela não cria uma nova conta interna. Em <b>Auditoria</b>, consulte quem fez mudanças e quando; clique em um evento para ver o detalhe. O último Diretor ativo não pode ser inativado.")
